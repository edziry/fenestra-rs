...
pub(super) fn method_attributes(source: &str, name: &str) -> BTreeSet<String> {
    let markers = [format!("pub fn {name}("), format!("pub const fn {name}(")];
    let offset = markers
        .iter()
...
pub(super) fn assert_method_surface(
    ...
    for name in expected {
        ...
        assert_eq!(
            ...
            must_use.contains(name),
            "must_use on {type_name}::{name}"
        );
        assert_eq!(
...
fn has_must_use(source: &str, item_offset: usize) -> bool {
    ...
    if prefix[line_start..].trim() == "pub const" {
